 ...
 * after a temporary disconnection; data channels use SCTP, which provides reliable and in-order communication, so
 * even if the state changes while the connection is temporarily interrupted the normal state update messages should
 * be received by the other participant once the connection is restored.
 */
public class LocalStateBroadcasterNoMcu extends LocalStateBroadcaster {
    ...
    private class IceConnectionStateObserver implements CallParticipantModel.Observer {
        ...
        private CallParticipantModel callParticipantModel;

        private PeerConnection.IceConnectionState iceConnectionState = null;
    ...
    public void destroy() {
        ...
        for (IceConnectionStateObserver iceConnectionStateObserver : iceConnectionStateObservers.values()) {
