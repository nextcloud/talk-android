    ...
    protected fun processChatMessageWebSocketMessage(jsonString: String) {
        fun parseChatMessages(jsonString: String): List<ChatMessageJson> {
            return try {
                ...
                val messageObj = eventObj.optJSONObject("message") ?: return emptyList()
                val dataObj = messageObj.optJSONObject("data") ?: return emptyList()
                val chatObj = dataObj.optJSONObject("chat") ?: return emptyList()
        ...
        val chatMessages = parseChatMessages(jsonString)
        if (chatMessages.isNotEmpty()) {
            conversationMessageNotifier.notifyMessagesReceived(chatMessages)
        }
    }
