...
fun GeocodingScreen(viewModel: GeoCodingViewModel, onBack: () -> Unit, onAddressSelected: (Address) -> Unit) {
    val results by viewModel.getGeocodingResultsLiveData().observeAsState(viewModel.getGeocodingResults())
    var queryText by rememberSaveable { mutableStateOf(viewModel.getQuery()) }
    val keyboardController = LocalSoftwareKeyboardController.current

    LaunchedEffect(Unit) {
        if (viewModel.getQuery().isNotEmpty() && viewModel.getGeocodingResults().isEmpty()) {
...
@Composable
private fun GeocodingScreenContent(query: String, results: List<String>, inputs: GeocodingScreenListenerInputs) {
        ...
        topBar = {
            TopAppBar(
                title = {
                    SearchField(
                        query = query,
                        onQueryChange = inputs.onQueryChange,
