...
public class MagicTrustManager implements X509TrustManager {
    ...
    public MagicTrustManager() {
        keystoreFile =
                new File(NextcloudTalkApplication.Companion.getSharedApplication().getDir("CertsKeystore",
    ...
    public void addCertInTrustStore(X509Certificate x509Certificate) {
        if (trustedKeyStore != null) {
            try {
                trustedKeyStore.setCertificateEntry(x509Certificate.getSubjectDN().getName(),
                        x509Certificate);
